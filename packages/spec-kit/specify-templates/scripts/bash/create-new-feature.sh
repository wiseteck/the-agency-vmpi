...
get_highest_from_branches() {
    ...
    # Capture output from the list_branches hook (empty if hook not defined)
    local branches
    branches=$(run_hook list_branches 2>/dev/null || echo "")
...
check_existing_branches() {
    ...
    # Sync remotes before listing branches (no-op if fetch_remotes hook not defined)
    run_hook fetch_remotes 2>/dev/null || true
...
# Source common functions (provides get_repo_root, run_hook, hook_defined, etc.)
